...
## Cursor Version Timeline (2026)

| Version | Date | Key Feature |
|---------|------|-------------|
| 2.4 | Jan 22 | Subagents, Skills, Cursor Blame (AI attribution in git blame) |
| 2.5 | Feb 17 | Plugin marketplace, long-running agents |
| 2.6 | Mar 3 | MCP Apps (interactive UIs), team marketplaces, Debug mode |
| — | Mar 4 | JetBrains IDE support via ACP |
| — | Mar 5 | Automations (always-on triggered agents) |
| — | Mar 11 | 30+ new plugins (Atlassian, Datadog, GitLab, etc.) |
| — | Mar 19 | Composer 2 (Cursor's own coding model) |

## Company Milestones
- 1M+ daily users, 50K+ businesses (Stripe, Figma, Salesforce)
- $1B+ annualized revenue (crossed Nov 2025)
- In talks to raise at ~$50B valuation
- Salesforce: 90% of 20K developers use Cursor
- Stripe: adoption went from single digits to 80%+
- ~35% of Cursor's own PRs generated by agents on VMs

## Parallel Agents
- Up to **20 cloud agents** in parallel (up from 8 at 2.0 launch)

## Tab Completion
- Tab completion is **free** (does not consume credits)
- "Predictive Editing" anticipates next several coding steps
- Speculative decoding with proprietary small models for ultra-low latency
